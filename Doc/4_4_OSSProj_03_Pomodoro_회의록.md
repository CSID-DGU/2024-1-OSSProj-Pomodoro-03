...
## 의제 목록
### - 중간발표 피드백 반영 사항
### - 디벨롭 진행사항 보고 및 방 만들기 구현
## 논의 내용 
### [ 중간 발표 피드백 반영 해야할 사항 ]
1. 팀 레포에 직접 작업한 부분 pr 기록작성을 위한 커밋 규칙 새로 작성하기
2. 이슈 관리 -> 제한사항 극복에 대한 내용 추가 / 이슈 유형 등록해서 분류 관리하기
### [ 프로젝트 디벨롭 진행과정 보고 ]
- 로그인과 프로필, 데이터 베이스에서 작성한 글 목록은 뜨지만 아직 CREATEROOM기능이 구현되지 않음 (지우)
- 자유게시판과 익명게시판의 글 구조가 쉐어 카테고리와 달라야하는데 아직 구현되지 않음 (지송)
- 현위치 뜨도록 구현 후 쉐어글의 위치도 핀으로 뜨도록 구현함 (나영)
...
## 참석자 : 김지우 김지송 박나영
## 의제 목록
### - 만약 채팅기능이 구현되지 않을 시 어떻게 만날지
### - 과연 직접 만나는 것만이 쉐어의 답일지 생각해보기
### - 공구룸과 비교되는 우리만의 구현 기능
## 논의 내용
### [ 채팅기능 구현되지 않을시의 대안 / 직접 만나기 or 대안 ]
- 쉐어글에 참여자가 모이면 채팅방을 개설해 채팅방 내에서 미팅장소와 시간을 정할 수 있도록 범위를 잡았음
- 생각보다 더뎌진 구현기능부분에서 채팅방까지 구현할수 있을지 미지수가 되어버림 -> 대안 필요
- 1) meeting 장소와 픽업시간을 설정할 수 있는 기능을 통해 만남 유도
- 2) 가상의 픽업 사물함을 생성해야하는가? -> 더 복잡하고 현하기 어려울 것으로 예상됨
결론 : 최대한 채팅방 개설을 시도(지송) 해보고 구현 실패시 1안을 가져가기로 결정
### [ 공구룸과의 차별성 정리 ]
- 지도상의 마커 눌렀을때 게시글 시각화
- 일반 커뮤니티의 구현
- 채팅방 개설을 통해 보다 원만한 미팅 장소와 시간, 가격을 정할 수 있음
